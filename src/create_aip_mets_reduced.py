...
from create_mets_md_csv import parseMetadata
from create_mets_rights import archivematicaGetRights
from create_mets_dspace_rights_mdref import archivematicaCreateMETSRightsDspaceMDRef
        ...
        with job.JobContext(logger=logger):
            try:
                ...
                if state.error_accumulator.error_count != 0:
                    job.set_status(
                        state.error_accumulator.error_count,
                        status_code="partial success",
                    )
                job.set_status(
                    state.error_accumulator.error_count, status_code="success"
                )
